...
void writeStructOnEEPROM(int actualRecord);             // escreve os dados da struct na EEPROM; actualRecord = apartir de que posição os dados seram reescritos
...
void winner_fx(){
  ...
  actualRecord = checkRecord();


  if (actualRecord != -1){
    printInfoWinner(winner, (actualRecord + 1));
  }
...
void fillRecordStruct(){
  ...
  for (i = 0; i < 10; i++){
    ...
    while (EEPROM.read((50 * i) + j) != '\0'){
      records[i].nome[j] = EEPROM.read((50 * i) + j);
      j++;
    } 
...
int checkRecord(){
  ...
  for (i = 0; i < 10; i++){
    ...
    if (actualRecord != -1){
      ...
      for (j = 0; j < 10; j++){
            records[i].data[j] = EEPROM.read(((50 * (i - 1)) + 40) + j);
        }
    
      }
      writeRecord(actualRecord);
      writeStructOnEEPROM(actualRecord);
      fillRecordStruct();
    }
...
void writeRecord(int actualRecord){
  ...
  for (int i = 0; i < 13; i++){
    records[actualRecord].nome[i] = newNome[i];
  }
...
// ---------------------------------------------------------
void writeStructOnEEPROM(int actualRecord){

  // escrevendo na EEPROM os valores alterados
  for (int i = actualRecord; i < 10; i++){
    int j = -1;
    
    do{
      j++;

      EEPROM.write(((50 * i) + j), records[i].nome[j]);
      
    } while (records[i].nome[j] != '\0');
  
    for (int j = 0; j < 7; j++){
      EEPROM.write((((50 * i) + 33) + j), records[i].record[j]);
    }
     for (int j = 0; j < 10; j++){
         EEPROM.write((((50 * i) + 40) + j), records[i].data[j]);
    }
  }
}
